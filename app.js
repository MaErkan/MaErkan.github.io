(() => {
  ...
  const PRAISE = ["أحسنتَ يا بني.", "بارك الله فيك.", "ممتاز، فكّرتَ تفكيرًا سليمًا.", "نعم، هذا هو المراد.", "إجابتك صحيحة.", "وُفِّقتَ في الاستنتاج.", "أحسنتَ التدبر.", "أصبتَ كبد الحقيقة.", "رائع، استمر.", "نعم يا فتى، هذا صحيح.", "لله درّك.", "هذا سياقٌ بيّن.", "أحسنتَ الاختيار.", "أحسنتَ الإجابة.", "فكّرتَ تفكير العالم."];
  const RETRY = ["حاول مرة أخرى يا بني.", "تأمل السياق ثم أجب.", "راجع المثال جيدًا.", "فكّر في دلالة الكلمة.", "اقترب، أعد المحاولة.", "انظر إلى الجملة كاملة."];
  ...
  function fb(ok, html) {
    const t = S.lastPraise || (ok ? "أحسنتَ يا بني." : "حاول مرة أخرى يا بني.");
    return `<div class="fb ${ok ? "ok" : "bad"}"><p class="t">${t}</p><div>${html}</div></div>`;
  }
  function play(ok) {
    const list = ok ? PRAISE : RETRY;
    const files = ok
      ? list.map((_, i) => "media/praise/p" + String(i).padStart(2, "0") + ".mp3?v=4")
      : list.map((_, i) => "media/praise/r" + String(i).padStart(2, "0") + ".mp3?v=4");
