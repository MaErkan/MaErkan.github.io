(() => {
  ...
  const PRAISE = ["بارك الله فيك يا بني.", "ما شاء الله، أحسنتَ.", "وفّقك الله إلى الصواب.", "لله درّك، إجابة نبيلة.", "زادك الله فهمًا يا فتى.", "فتح الله عليك.", "جزاك الله خيرًا.", "أكرمك الله بهذا الفهم.", "نعم والله، هذا هو المراد.", "نفع الله بك يا بني.", "هداك الله إلى السياق.", "أيّدك الله، أصبتَ.", "طيّب الله بيانك.", "سبحان الله، فكّرتَ تفكير العلماء.", "تولّاك الله، وُفِّقتَ."];
  const RETRY = ["وفّقك الله، حاول مرة أخرى.", "تأمل السياق يا بني.", "راجع المثال، سدّدك الله.", "فكّر في دلالة الكلمة.", "اقترب، أعد المحاولة.", "انظر إلى الجملة كاملة، وفقك الله."];
  ...
  function fb(ok, html) {
    const t = S.lastPraise || (ok ? "بارك الله فيك يا بني." : "وفّقك الله، حاول مرة أخرى.");
    return `<div class="fb ${ok ? "ok" : "bad"}"><p class="t">${t}</p><div>${html}</div></div>`;
  }
  function play(ok) {
    const list = ok ? PRAISE : RETRY;
    const files = ok
      ? list.map((_, i) => "media/praise/p" + String(i).padStart(2, "0") + ".mp3?v=5")
      : list.map((_, i) => "media/praise/r" + String(i).padStart(2, "0") + ".mp3?v=5");
  ...
  function intro() {
    return `
      ...
      <div class="stage">
        <video id="v" poster="media/cover.jpg?v=1" playsinline preload="metadata" src="media/v-intro.mp4?v=25" ${S.started ? "controls" : ""}></video>
